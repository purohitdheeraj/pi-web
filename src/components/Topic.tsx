...
const Topic = ({ slug }: { slug: string }) => {
  ...
  // List of topics (static in this case, but can be fetched dynamically from an API)
  const topics = [
  ...
  if (!topic) {
    return <div className='space-y-8 p-6 mt-8 mx-auto  w-full flex-grow no-scrollbar overflow-y-auto'>Topic not found</div>;
  }

  return (
    <>
      <div className="space-y-8 p-6 mt-8 mx-auto  w-full flex-grow no-scrollbar overflow-y-auto ">
        ...
        {/* Header Card */}
        <Fade triggerOnce>
          <div className="bg-transparent px-3 border rounded-3xl p-6 flex flex-col md:flex-row gap-6 items-end">
